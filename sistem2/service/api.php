...
$path = str_replace('/sistem2/service/api.php', '', parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH));
$path = trim($path, '/');
...
if ($method === 'POST' && $pathParts[0] === 'tracking' && $pathParts[1] === 'check') {
    ...
    if ($identifier) {
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            ...
            echo json_encode([
                "error" => "Invalid JSON",
                "debug_error" => json_last_error_msg()
            ]);
            exit;
        }

        $identifier = $input['orderId'] ?? $input['no_resi'] ?? null;
